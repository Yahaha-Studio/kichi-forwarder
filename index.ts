...
const PRESENCE_SCOPES = ["current", "today"] as const;
...
type PresenceScope = typeof PRESENCE_SCOPES[number];
type IdlePlanAction = {
  poseType: PoseType;
  action: string;
...
function isPresenceScope(value: unknown): value is PresenceScope {
...
function resolveMateDailySchedule(
...
function createKichiQueryStatusTool(
  service: KichiForwarderService,
  includeDailySchedule: boolean,
) {
  return {
    name: "kichi_query_status" as const,
        ...
        ...(includeDailySchedule
          ? {
              presenceScope: {
                ...
                description:
                  "Use \"current\" for questions about your current location or activity. Use \"today\" for questions about today's plans. Otherwise omit this field.",
              },
            }
          : {}),
      },
    ...
    execute: async (_toolCallId: string, params: unknown) => {
      const { requestId, presenceScope: rawPresenceScope } = (params || {}) as {
        requestId?: unknown;
        presenceScope?: unknown;
      };
      if (requestId !== undefined && typeof requestId !== "string") {
        return jsonResult({ success: false, error: "requestId must be a string when provided" });
      }
      let presenceScope: PresenceScope | undefined;
      ...
      let result: QueryStatusResultPayload;
      try {
