...
const botActivityCooldowns = new Map();
function handleBotActivity(api, service, msg) {
    ...
    if (now - lastReply < BOT_ACTIVITY_COOLDOWN_MS)
        return;
    ...
    const message = `[Collaborative activity with ${msg.fromName}]\nActivity: ${activity.goal}\nTurn ${activity.turn + 1}/${activity.maxTurns} (${turnsLeft} turns left)\n\nConversation so far:\n${historyLines.join("\n")}\n\nReply naturally (1-2 sentences). Stay on topic with the activity goal. ${turnsLeft <= 1 ? "This is the last turn — wrap up the activity with a conclusion." : ""}Just output your reply, nothing else.`;
...
const plugin = {
    ...
    register(api) {
        ...
        api.registerTool(createAgentScopedTool(runtimeManager, (service) => ({
            ...
            execute: async (_toolCallId, params) => {
                const { toAvatarId, activityType, goal, bubble, maxTurns, poseType, action } = (params || {});
                if (typeof toAvatarId !== "string" || !toAvatarId.trim()) {
                    return jsonResult({ success: false, error: "toAvatarId is required" });
                }
