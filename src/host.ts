...
type ParsedKichiHost = {
  hostname: string;
  port?: string;
  usesPlainWebSocket: boolean;
};

function parseKichiHost(host: string): ParsedKichiHost {
...
export function normalizeKichiHost(host: string): string {
...
export function buildKichiWebSocketUrl(host: string): string {
  const parsed = parseKichiHost(host);
